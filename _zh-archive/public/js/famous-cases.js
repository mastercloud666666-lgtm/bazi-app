  ...
  function render() {
    ...
    grid.innerHTML = FAMOUS_CASES.map((c) => {
      ...
      const cols = b ? ['year', 'month', 'day', 'hour'].map((k) =>
        `<div class="fc-pillar"><span class="fc-tg" data-bazi-symbol="${b[k].tg}">${b[k].tg}</span><span class="fc-dz" data-bazi-symbol="${b[k].dz}">${b[k].dz}</span></div>`
      ).join('') : '';
    ...
    window.BaziCalc.colorizeSymbols(grid);
  }
