...
第一部分：性格分析
先用一句话说明日主和格局（如"你是丙火日主，身强，偏印格"），再说2-3个具体性格特点，每个特点单独一段，结合格局和日柱说，用具体表现方式描述。最后一段说一个需要注意的性格短板。

${hasSpecial ? `第二部分：特殊流年提醒
以下年份存在天克地冲或岁运并临，请逐年分析：
${special_years_text}

对每个年份：先说这是哪种类型（天克地冲还是岁运并临），再根据日主五行和格局，具体说这一年容易在哪些方面出现变动或需要注意（如感情、工作变动、健康、财务、家庭等），给1-2条实际建议。已过的年份简短提一句即可，今年和未来的年份要重点说。` : ''}

用口语，用"你"称呼对方，两部分之间空一行，不写标题符号，不写诗，不引用古文，直接从第一部分开始，说完就结束。`;
      } else {
